...
После того как пользователь нажимает кнопку `Поддержка` или пишет команду `/support`. На стороне сотрудника это выглядит
так:

![Новый запрос](/images/support/support-new-ticket.PNG)

<br>
У каждого сотрудника есть доступ к меню со всеми его тикетами. По которым он может бегать и управлять
...
Да много чего еще можно рассказать про этот проект, но всегда нужно знать меру. Это был мой первый заказ на «фрилансе»
и я его выполнил идеально.